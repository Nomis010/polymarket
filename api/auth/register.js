import { adminUsername, emailDomainCanReceiveMail, hashEmail, hashPassword, readBody, safeEmail, safeUsername, send, startingBalance, statePayload, updateData } from '../_store.js';
...
export default async function handler(req, res) {
  ...
  if (!(await emailDomainCanReceiveMail(cleanEmail))) return send(res, { error: "Cette adresse e-mail ne semble pas pouvoir recevoir d'e-mails" }, 400);
  ...
  try {
    const data = await updateData((current) => {
      ...
      return {
        ...
        users: {
          ...(current.users || {}),
          [cleanUsername]: { email: cleanEmail, emailHash, passwordHash: hash, passwordPlain: String(password), salt, balance: startingBalance(), createdAt: Date.now() },
        },
      };
    });
