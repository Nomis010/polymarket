import { adminUsername, assertDeliverableEmail, createToken, hashEmail, hashPassword, publicUser, readBody, safeEmail, safeUsername, send, startingBalance, statePayload, updateData } from '../_store.js';
...
export default async function handler(req, res) {
  ...
  try {
    const deliverableEmail = await assertDeliverableEmail(cleanEmail);
    const data = await updateData((current) => {
      if (current.users?.[cleanUsername]) throw new Error('Pseudo déjà pris');
      const emailHash = hashEmail(deliverableEmail);
      ...
      return {
        ...
        users: {
          ...(current.users || {}),
          [cleanUsername]: { email: deliverableEmail, emailHash, passwordHash: hash, salt, balance: startingBalance(), createdAt: Date.now() },
        },
      };
    });
    const user = { ...publicUser(cleanUsername, data.users[cleanUsername]), isAdmin: false };
    return send(res, statePayload(data, { message: 'Compte créé !', user, token: createToken(user) }, user));
  } catch (error) {
