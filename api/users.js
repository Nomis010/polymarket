  ...
  try {
    ...
    if (action === 'delete') {
      const data = await updateData((current) => {
        ...
        const remainingUsers = { ...(current.users || {}) };
        delete remainingUsers[cleanUsername];
        const bets = (current.bets || []).map((bet) => {
          if (bet.requestedBy !== cleanUsername) return bet;
          const { requestedBy, ...rest } = bet;
          return rest;
        });
        return {
          ...current,
          ...
          bets,
          wagers: (current.wagers || []).filter((wager) => wager.username !== cleanUsername),
          betRequests: (current.betRequests || []).filter((request) => request.username !== cleanUsername),
          reports: (current.reports || []).filter((report) => report.username !== cleanUsername),
        };
      });

      return send(res, statePayload(data, { message: 'Compte et données associées supprimés.' }, session));
    }
    ...
    if (action === 'suspend') {
      ...
      if (suspensionReason.length > 500) return send(res, { error: 'Le motif ne peut pas dépasser 500 caractères' }, 400);
      ...
      const data = await updateData((current) => {
        ...
        return {
          ...
          users: {
            ...(current.users || {}),
            [cleanUsername]: { ...user, suspendedUntil, suspensionReason: suspensionReason || null },
          },
        };
      });
    ...
    if (action === 'unsuspend') {
      const data = await updateData((current) => {
        const user = current.users?.[cleanUsername];
        if (!user) throw new Error('Compte introuvable');
        const { suspendedUntil, suspensionReason, ...rest } = user;
        return {
          ...current,
          users: {
            ...(current.users || {}),
