  ...
  try {
    ...
    if (action === 'ban') {
      const data = await updateData((current) => {
        ...
        if (user.bannedAt) throw new Error('Ce compte est déjà banni définitivement');
        return {
          ...current,
          users: {
            ...(current.users || {}),
            [cleanUsername]: { ...user, bannedAt: Date.now() },
          },
        };
      });

      return send(res, statePayload(data, { message: 'Compte banni définitivement.' }, session));
    }
