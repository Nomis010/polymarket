...
const QUOTE_RESERVE = 500;

const emptyData = () => ({ users: {}, bets: [], wagers: [], betRequests: [], reports: [] });

export function send(res, payload, status = 200) {
  // State is personalised and changes as players place wagers. It must never
  // be served from a browser or Vercel edge cache.
...
export function publicUser(username, data, viewer = null) {
  const user = {
    username,
  ...
  if (viewer?.isAdmin) {
    ...
    user.emailHash = data.emailHash || null;
    user.suspendedUntil = data.suspendedUntil || null;
    user.suspensionReason = data.suspensionReason || null;
  }
  return user;
}
...
export function publicState(data, viewer = null) {
  return {
    users: Object.fromEntries(Object.entries(data.users || {}).map(([username, user]) => [username, publicUser(username, user, viewer)])),
    bets: data.bets || [],
    betRequests: viewer ? (data.betRequests || []).filter((request) => viewer.isAdmin || request.username === viewer.username) : [],
    reports: viewer ? (data.reports || []).filter((report) => viewer.isAdmin || report.username === viewer.username) : [],
...
export function suspensionMessage(user) {
  const until = new Date(user.suspendedUntil).toLocaleString('fr-FR');
  const reason = String(user.suspensionReason || '').trim();
  return `Connexion bloquée jusqu’au ${until}${reason ? ` · Motif : ${reason}` : ''}`;
}
...
export async function assertDeliverableEmail(email) {
  ...
  const match = cleanEmail.match(/^[^\s@]+@([^\s@]+\.[^\s@]+)$/);
  if (!match) throw new Error('E-mail invalide');

  const domain = match[1];
  try {
    const mx = await dns.resolveMx(domain);
    if (mx?.length) return cleanEmail;
  } catch {
    // Some domains receive mail on their A/AAAA record without publishing MX.
  }

  try {
    const addresses = await dns.lookup(domain);
    if (addresses?.address) return cleanEmail;
  } catch {
    throw new Error("Le domaine de cet e-mail n'existe pas ou ne reçoit pas de mail");
  }
...
export function getVirtualStakes(bet) {
  ...
  return options.map((_, index) => {
    ...
    return Number.isFinite(value) && value > 0 ? value : QUOTE_RESERVE;
  });
}
...
export function getOptionOdds(bet, wagers, optionIndex) {
  ...
  const optionStake = getOptionStake(wagers, bet.id, optionIndex) + Number(virtualStakes[optionIndex] || QUOTE_RESERVE);
  const quote = ((totalPlayerStake + totalVirtualStake) / optionStake) * 0.9;
  return Number(Math.max(1.1, quote).toFixed(2));
}
