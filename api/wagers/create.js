...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!wagerAmount || wagerAmount < 1) return send(res, { error: 'Mise minimum : 1 seeonium' }, 400);
    ...
    const data = await updateData((current) => {
      ...
      if (user.bannedAt) throw new Error('Ce compte est banni définitivement');
    ...
    return send(res, statePayload(data, { message: 'Mise de ' + wagerAmount + ' seeonium placée !' }, session));
  } catch (error) {
