...
export default async function handler(req, res) {
  ...
  try {
    ...
    const data = await updateData((current) => {
      ...
      if ((current.wagers || []).some((w) => w.betId === betId && w.username === session.username)) throw new Error('Tu as déjà misé sur ce paris');
      return {
        ...current,
        users: { ...(current.users || {}), [session.username]: { ...user, balance: user.balance - wagerAmount } },
        wagers: [ ...(current.wagers || []), { id: id(), betId, username: session.username, amount: wagerAmount, optionIndex: optIdx, createdAt: Date.now() } ],
      };
    });
    return send(res, statePayload(data, { message: 'Mise de ' + wagerAmount + ' 🪙 placée !' }));
  } catch (error) {
