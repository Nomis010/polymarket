...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { title, description, options, virtualStakes, closesAt } = readBody(req);
    ...
    if (!Array.isArray(virtualStakes) || virtualStakes.length !== options.length || virtualStakes.some((o) => Number.isNaN(Number(o)) || Number(o) < 0)) return send(res, { error: 'Mise fictive invalide' }, 400);

    const bet = {
      id: id(), title: title.trim(), description: String(description || '').trim(),
      options: options.map((o) => String(o).trim()), virtualStakes: virtualStakes.map((o) => Number(Number(o).toFixed(0))),
