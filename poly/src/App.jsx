...
const CurrencyIcon = ({ small = false }) => (
  <img
    className={`${styles.currencyIcon} ${small ? styles.currencyIconSmall : ''}`}
    src={seeoniumIcon}
    alt={CURRENCY_NAME}
  />
);
...
export default function App() {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <header className={styles.header}>
        ...
        {currentUser && (
          <nav className={styles.nav}>
            {!currentUser.isAdmin && (
              <span className={styles.balance}>{getUserBalance()?.toLocaleString('fr-FR')} <CurrencyIcon small /></span>
            )}
            {currentUser.isAdmin && (
                ...
                <div key={bet.id} className={styles.betCard}>
                  ...
                  {myWagers.length > 0 && (
                    <div className={styles.myWager}>
                      ...
                      {myWagers.map((w) => (
                        <span key={w.id}>
                          {w.amount} {CURRENCY_NAME} sur "{bet.options[w.optionIndex]}" à x{Number(w.lockedOdds || bet.odds?.[w.optionIndex] || 1).toFixed(2)}
                          {bet.status === 'resolved' && (
                            w.optionIndex === bet.resolvedOption
                              ? <> · gain {Math.floor(w.amount * Number(w.lockedOdds || bet.odds?.[w.optionIndex] || 1))} {CURRENCY_NAME}</>
                              : <> · perdu</>
                          )}
                        </span>
                      ))}
                  ...
                  {bet.status === 'open' && (
                    <div className={styles.wagerRow}>
                      ...
                      <input type="number" min="1" max={getUserBalance()} placeholder={`Mise en ${CURRENCY_NAME}`} style={{ flex: 1, minWidth: 90 }}
      ...
      {view === 'leaderboard' && (
        <div className={styles.page}>
          <h2 className={styles.pageTitle}>Classement {APP_NAME}</h2>
          <div className={styles.card}>
            ...
            {leaderboard.map((u, i) => (
              <div key={u.username} className={styles.leaderRow}
                ...
                <span className={styles.leaderBalance}>{u.balance.toLocaleString('fr-FR')} <CurrencyIcon small /></span>
              </div>
            ))}
          </div>
        ...
        <div className={styles.page}>
          <div className={styles.statsGrid}>
            {[
              ...
              { label: `${CURRENCY_NAME} misés`, value: totalWagered.toLocaleString('fr-FR'),     emoji: 'S' },
              ...
              <div key={bet.id} className={styles.betCard}>
                <div className={styles.betCardTop}>
                  ...
                  <div className={styles.poolBox}>
                    ...
                    <div className={styles.poolValue}>{tp.toLocaleString('fr-FR')} <CurrencyIcon small /></div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', margin: '8px 0' }}>
                  {bet.options.map((opt, i) => {
                    ...
                    const isWin = bet.resolvedOption === i;
                    return (
                      <span key={i} style={{ background: isWin ? 'rgba(42, 205, 149, 0.16)' : 'var(--bg-secondary)', border: isWin ? '1px solid rgba(124, 248, 200, 0.55)' : '1px solid var(--border)', borderRadius: 6, padding: '3px 10px', fontSize: 12, color: 'var(--text-secondary)' }}>
            ...
            <div className={styles.playersGrid}>
              {Object.entries(users).sort((a, b) => b[1].balance - a[1].balance).map(([u, d]) => (
                <div key={u} className={styles.playerCard}>
                  ...
                  <div style={{ fontWeight: 700, color: 'var(--gold)', fontSize: 14 }}>{d.balance.toLocaleString('fr-FR')} <CurrencyIcon small /></div>
