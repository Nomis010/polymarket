    ...
    <div className={styles.optionEditor}>
      {form.options.map((option, index) => (
        <div key={index} className={`${styles.optionRow} ${withStake ? '' : styles.optionRowFixed}`}>
          <input value={option} readOnly placeholder={`Option ${index + 1}`} onChange={(event) => updateOption(setter, 'options', index, event.target.value)} />
          {withStake && (
              ...
              {selectedWagers.map((wager) => {
                ...
                return (
                  <div key={wager.id} className={styles.historyRow}>
                    ...
                    <span>{bet?.title || 'Pari supprimé'} · {bet?.options?.[wager.optionIndex] || 'Option'}</span>
