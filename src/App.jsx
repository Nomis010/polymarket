...
export default function App() {
  ...
  useEffect(() => {
    ...
    const updateMobileNavigation = () => {
      ...
      frameId = window.requestAnimationFrame(() => {
        setIsMobileNavCollapsed(window.innerWidth <= 760 && window.scrollY > 72);
        frameId = null;
      });
    };
    ...
    return () => {
      window.removeEventListener('scroll', updateMobileNavigation);
  ...
  const renderNav = () => currentUser && (
    <nav className={`${styles.railNav} ${isMobileNavCollapsed ? styles.railNavCollapsed : ''}`} aria-label="Navigation principale">
      {navItems.map((item) => (
      ...
      {isMobileNavCollapsed && (
        <button className={styles.menuToggle} onClick={() => setIsMobileNavCollapsed(false)} aria-label="Ouvrir le menu">
          <Icon name="menu" />
        </button>
      )}
    </nav>
