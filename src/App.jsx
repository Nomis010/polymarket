                ...
                <div key={bet.id} className={styles.betCard}>
                  ...
                  {myWagers.length > 0 && (
                    <div className={styles.myWager}>
                      ...
                      {myWagers.map((w) => (
                        <span key={w.id}>
                          {w.amount} 🪙 sur "{bet.options[w.optionIndex]}" à x{Number(w.lockedOdds || bet.odds?.[w.optionIndex] || 1).toFixed(2)}
                          {bet.status === 'resolved' && (
                            w.optionIndex === bet.resolvedOption
                              ? <> · gain {Math.floor(w.amount * Number(w.lockedOdds || bet.odds?.[w.optionIndex] || 1))} 🪙</>
                              : <> · perdu</>
                          )}
                        </span>
                      ))}
