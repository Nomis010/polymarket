...
const DEFAULT_VIRTUAL_STAKE = 500;
const YES_NO_LABELS = ['OUI', 'NON'];
...
const fmtDateTimeLocalInput = (ts) => {
  if (!ts) return '';
...
const formatOptionLabel = (option, index) => {
  const clean = String(option || '').trim().toUpperCase();
  if (clean === 'OUI' || index === 0) return 'OUI';
  if (clean === 'NON' || index === 1) return 'NON';
  return clean || `OPTION ${index + 1}`;
};
...
const getOptionOdds = (bet, wagers, optionIndex) => {
  ...
  return Math.max(1.1, quote).toFixed(2);
};
...
export default function App() {
  ...
  const [isMobileNavCollapsed, setIsMobileNavCollapsed] = useState(() => typeof window !== 'undefined' && window.innerWidth <= 760);
  ...
  const refreshState = async (token = sessionToken) => {
    ...
    // Do not let an older background response overwrite a state received from
    // a wager or another action completed while it was in flight.
    if (stateRevisionRef.current === requestRevision) applyState(data);
    return data;
  };
  ...
  useEffect(() => {
    if (!sessionToken || view === 'loading' || view === 'login') return undefined;
    ...
    const refreshSilently = async () => {
      ...
      refreshing = true;
      try {
        await refreshState(sessionToken);
      } catch (error) {
        if (error.message === 'Ce compte a été supprimé.') {
          clearSession();
          setSessionToken(null);
          setCurrentUser(null);
          setView('login');
          notify('Ton compte a été supprimé.', 'error');
          return;
        }
        // A transient background failure must not interrupt the person using
        // the app. The next interval (or focus) will try again.
        console.warn('Background state refresh failed:', error);
      } finally {
        refreshing = false;
      }
    };
    ...
    const intervalId = window.setInterval(refreshSilently, BACKGROUND_REFRESH_MS);
  ...
  useEffect(() => {
    const closeMobileNavigation = () => {
      if (window.innerWidth <= 760) setIsMobileNavCollapsed(true);
    };
    const syncMobileNavigation = () => setIsMobileNavCollapsed(window.innerWidth <= 760);

    window.addEventListener('scroll', closeMobileNavigation, { passive: true });
    window.addEventListener('resize', syncMobileNavigation);
    syncMobileNavigation();
    return () => {
      window.removeEventListener('scroll', closeMobileNavigation);
  ...
  const validateOptions = (options) => Array.isArray(options)
    && options.length === 2
    && options[0]?.trim().toUpperCase() === 'OUI'
    && options[1]?.trim().toUpperCase() === 'NON';

  const handleCreateBet = async () => {
    if (!newBet.title.trim()) return notify('Titre requis', 'error');
    if (!validateOptions(newBet.options)) return notify('Les réponses doivent être OUI et NON', 'error');
  ...
  const handleRequestBet = async () => {
    ...
    if (!validateOptions(requestForm.options)) return notify('Les réponses doivent être OUI et NON', 'error');
    try {
  ...
  const handleSelectUser = (username) => {
    ...
    setSuspendDraft(fmtDateTimeLocalInput(users[username]?.suspendedUntil));
    setSuspendReason(users[username]?.suspensionReason || '');
  };
  ...
  const handleSuspendUser = async () => {
    if (!selectedUser) return;
    if (!suspendDraft) return notify('Choisis une date de fin de blocage', 'error');
    try {
      const data = await api('/api/users', { method: 'POST', token: sessionToken, body: { action: 'suspend', username: selectedUser, suspendedUntil: suspendDraft, suspensionReason: suspendReason } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
  ...
  const handleUnsuspendUser = async () => {
    if (!selectedUser) return;
    try {
      const data = await api('/api/users', { method: 'POST', token: sessionToken, body: { action: 'unsuspend', username: selectedUser } });
      syncFromApi(data);
      setSuspendDraft('');
      setSuspendReason('');
    } catch (error) {
  ...
  const handleDeleteUser = async () => {
    if (!selectedUser) return;
    if (!window.confirm(`Supprimer définitivement le compte « ${selectedUser} », ses mises, demandes et signalements ? Cette action est irréversible.`)) return;
    try {
      const data = await api('/api/users', { method: 'POST', token: sessionToken, body: { action: 'delete', username: selectedUser } });
      syncFromApi(data);
      setSelectedUser(null);
    } catch (error) {
      notify(error.message, 'error');
  ...
  const handleDeleteWager = async (wagerId) => {
    if (!window.confirm('Supprimer cette mise de l’historique ?')) return;
    try {
      const data = await api('/api/users', { method: 'POST', token: sessionToken, body: { action: 'delete-wager', username: selectedUser, wagerId } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
  ...
  const navItems = currentUser?.isAdmin
    ? [
      ...
      { key: 'leaderboard', label: 'Classement', icon: 'crown', action: () => { setView('admin'); setAdminView('leaderboard'); } },
  ...
  const renderNav = () => currentUser && (
    <nav className={`${styles.railNav} ${isMobileNavCollapsed ? styles.railNavCollapsed : styles.railNavExpanded}`} aria-label="Navigation principale">
      {navItems.map((item) => (
      ...
      <button
        ...
        onClick={() => setIsMobileNavCollapsed((collapsed) => !collapsed)}
        aria-label={isMobileNavCollapsed ? 'Ouvrir le menu' : 'Fermer le menu'}
      >
        <Icon name={isMobileNavCollapsed ? 'menu' : 'x'} />
      </button>
    </nav>
  );
  ...
  const renderOptionEditor = ({ form, setter, withStake = false }) => (
    <div className={styles.optionEditor}>
      {form.options.map((option, index) => (
        <div key={index} className={`${styles.optionRow} ${withStake ? '' : styles.optionRowFixed}`}>
          <input value={formatOptionLabel(option, index)} readOnly placeholder={`Option ${index + 1}`} onChange={(event) => updateOption(setter, 'options', index, event.target.value)} />
          {withStake && (
      ...
      <p className={styles.hint}>Les paris Polyscout26 acceptent uniquement deux réponses : <strong>OUI</strong> ou <strong>NON</strong>.</p>
    </div>
  );
  ...
  const renderBetCard = (bet, admin = false) => {
    ...
    const selectedOptionIndex = Number.parseInt(wagerForm[bet.id]?.option, 10);
    const selectedOdds = Number.isInteger(selectedOptionIndex) && bet.options[selectedOptionIndex]
      ? Number(getOptionOdds(bet, betWagers, selectedOptionIndex))
      : null;
    const enteredAmount = Number.parseInt(wagerForm[bet.id]?.amount, 10);
    const potentialGain = selectedOdds && enteredAmount > 0 ? Math.floor(enteredAmount * selectedOdds) : null;
    const totalMyWager = myWagers.reduce((sum, wager) => sum + Number(wager.amount || 0), 0);
    ...
    return (
      <div key={bet.id} className={styles.betCard}>
        <div className={styles.betCardTop}>
          <div>
            <div className={styles.betStatusRow}>
              <span className={styles.tag} style={{ background: tag.bg, color: tag.color }}>{tag.label}</span>
              {bet.closesAt && bet.status === 'open' && <span className={styles.closingDate}>Ferme le {fmtDate(bet.closesAt)}</span>}
            </div>
        ...
        <div className={styles.optionsGrid} style={{ gridTemplateColumns: `repeat(${bet.options.length}, 1fr)` }}>
          {bet.options.map((option, index) => {
            ...
            const isSelectable = !admin && bet.status === 'open';
            const isSelected = String(wagerForm[bet.id]?.option) === String(index);
            const OptionCard = isSelectable ? 'button' : 'div';
            return (
              <OptionCard
                key={index}
                type={isSelectable ? 'button' : undefined}
                className={`${styles.optionCard} ${isWin ? styles.optionWinner : ''} ${isSelectable ? styles.optionCardSelectable : ''} ${isSelected ? styles.optionCardSelected : ''}`}
                onClick={isSelectable ? () => setWagerForm((form) => {
                  const currentOption = String(form[bet.id]?.option);
                  const nextWager = { ...(form[bet.id] || {}) };
                  if (currentOption === String(index)) delete nextWager.option;
                  else nextWager.option = String(index);
                  return { ...form, [bet.id]: nextWager };
                }) : undefined}
                aria-pressed={isSelectable ? isSelected : undefined}
              >
                <div className={styles.optionName}>{formatOptionLabel(option, index)}</div>
        ...
        {!admin && myWagers.length > 0 && (
          <div className={styles.myWager}>
            ...
            {myWagers.map((wager) => (
              <span key={wager.id}>
                {compactCurrency(wager.amount)} {CURRENCY_NAME} sur "{formatOptionLabel(bet.options[wager.optionIndex], wager.optionIndex)}" à x{Number(wager.lockedOdds || getOptionOdds(bet, betWagers, wager.optionIndex)).toFixed(2)}
        ...
        {!admin && bet.status === 'open' && (
          <div className={styles.wagerRow}>
            <label className={styles.wagerAmountField}>
              <span className={styles.wagerLabel}>Ta mise</span>
            ...
            <div className={styles.potentialGain} aria-live="polite">
              <span>Gain potentiel</span>
              <strong>{potentialGain ? `${compactCurrency(potentialGain)} ${CURRENCY_NAME}` : '—'}</strong>
            </div>
        ...
        {admin && (
          <div className={styles.adminActions}>
            ...
            {(bet.status === 'open' || bet.status === 'closed') && bet.options.map((option, index) => (
              <button key={index} className={styles.primaryBtn} onClick={() => handleResolveBet(bet.id, index)}>Valider {formatOptionLabel(option, index)}</button>
            ))}
        ...
        <div className={styles.betMeta}>
          ...
          {!admin && totalMyWager > 0 && <span className={styles.myWagerTotal}>Déjà misé par toi : {compactCurrency(totalMyWager)} {CURRENCY_NAME}</span>}
        </div>
      </div>
    );
  };
      ...
      {betRequests.map((request) => {
        ...
        return (
          <div key={request.id} className={styles.betCard}>
            ...
            <p className={styles.betDesc}>{request.options.map((option, index) => formatOptionLabel(option, index)).join(' · ')}</p>
  ...
  const renderProfile = () => (
    <main className={styles.mainColumn}>
      ...
      <div className={styles.card}>
        <button className={styles.logoutProfileBtn} onClick={handleLogout}>
          <Icon name="logout" />
          Se déconnecter
        </button>
      </div>
            ...
            <div className={styles.requestOptions}>
              {request.options.map((option, index) => (
                <div key={index} className={styles.requestOption}>
                  <strong>{formatOptionLabel(option, index)}</strong>
          ...
          {selected && (
            <>
              ...
              <div className={styles.detailGrid}>
                ...
                <div><span>Connexion</span><strong>{selected.suspendedUntil && selected.suspendedUntil > Date.now() ? `Bloqué jusqu’au ${fmtDate(selected.suspendedUntil)}` : 'Autorisée'}</strong></div>
              </div>
              <div className={styles.field}>
              ...
              <div className={styles.field}>
                <label className={styles.label}>Empêcher la connexion jusqu’au</label>
                <input type="datetime-local" value={suspendDraft} onChange={(event) => setSuspendDraft(event.target.value)} />
              </div>
              <div className={styles.field}>
                <label className={styles.label}>Motif du blocage (visible à la connexion)</label>
                <textarea value={suspendReason} maxLength="500" onChange={(event) => setSuspendReason(event.target.value)} placeholder="Ex. Non-respect des règles de Polyscout26." />
              </div>
              <div className={styles.adminActions}>
                <button className={styles.primaryBtn} onClick={handleUpdateBalance}>Mettre à jour</button>
                <button className={styles.dangerBtn} onClick={handleSuspendUser}>Bloquer temporairement</button>
                {selected.suspendedUntil && selected.suspendedUntil > Date.now() && <button className={styles.secondaryBtn} onClick={handleUnsuspendUser}>Réautoriser la connexion</button>}
                <button className={styles.dangerBtn} onClick={handleDeleteUser}>Supprimer le compte</button>
              </div>
              ...
              {selectedWagers.map((wager) => {
                ...
                return (
                  <div key={wager.id} className={styles.historyRow}>
                    <strong>{compactCurrency(wager.amount)} {CURRENCY_NAME}</strong>
                    <span>{bet?.title || 'Pari supprimé'} · {formatOptionLabel(bet?.options?.[wager.optionIndex], wager.optionIndex)}</span>
                    <button className={styles.dangerBtn} onClick={() => handleDeleteWager(wager.id)}>Supprimer cette mise</button>
                  </div>
                );
              })}
            </>
  ...
  const renderPage = () => {
    ...
    if (currentUser?.isAdmin) {
      ...
      if (adminView === 'leaderboard') return renderLeaderboard();
  ...
  return (
    <div className={`${styles.wrapper} ${currentUser ? styles.appShell : ''}`}>
      ...
      {showRulesModal && (
        <div className={styles.modalBackdrop} role="dialog" aria-modal="true" aria-labelledby="rules-title">
          <div className={styles.modalCard}>
            <h2 id="rules-title" className={styles.cardTitle}>Règles importantes</h2>
            <p>Polyscout26 est réservé aux animateurs de la 26ème et n’est pas accessible aux animés.</p>
            <p>Miser sur un pari qui nous concerne directement (délit d’initié) n’est pas autorisé.</p>
            <button className={styles.primaryBtn} onClick={() => setShowRulesModal(false)}>J’ai compris</button>
          </div>
        </div>
      )}

      {currentUser && renderBrand()}
