...
export default function App() {
  ...
  const handleSuspendUser = async () => {
    ...
    try {
      const data = await api('/api/users', { method: 'POST', token: sessionToken, body: { action: 'suspend', username: selectedUser, suspendedUntil: suspendDraft } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
        ...
        {!admin && bet.status === 'open' && (
          <div className={styles.wagerRow}>
            <div className={styles.choiceField}>
              <span className={styles.wagerLabel}>Choisis une option</span>
              <div className={styles.choiceButtons} role="group" aria-label="Choisir une réponse">
                {bet.options.map((option, index) => (
                  <button
                    key={`${bet.id}-${index}`}
                    className={`${styles.choiceBtn} ${String(wagerForm[bet.id]?.option) === String(index) ? styles.choiceBtnActive : ''}`}
                    onClick={() => setWagerForm((form) => ({ ...form, [bet.id]: { ...form[bet.id], option: String(index) } }))}
                  >
                    <strong>{formatOptionLabel(option, index)}</strong>
                    <span>x{getOptionOdds(bet, betWagers, index)}</span>
                  </button>
                ))}
              </div>
            </div>
