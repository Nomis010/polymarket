...
export default function App() {
  ...
  useEffect(() => {
    ...
    const refreshSilently = async () => {
      ...
      try {
        ...
        if (error.message === 'Ce compte est banni définitivement.') {
          clearSession();
          setSessionToken(null);
          setCurrentUser(null);
          setView('login');
          notify('Ton compte a été banni définitivement.', 'error');
          return;
        }
  ...
  const handleBanUser = async () => {
    if (!selectedUser) return;
    if (!window.confirm(`Bannir définitivement le compte « ${selectedUser} » ? Cette action ne peut pas être annulée depuis l’administration.`)) return;
    try {
      const data = await api('/api/users', { method: 'POST', token: sessionToken, body: { action: 'ban', username: selectedUser } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
          ...
          {selected && (
            <>
              ...
              <div className={styles.detailGrid}>
                ...
                <div><span>Connexion</span><strong>{selected.bannedAt ? `Banni définitivement le ${fmtDate(selected.bannedAt)}` : selected.suspendedUntil && selected.suspendedUntil > Date.now() ? `Bloqué jusqu’au ${fmtDate(selected.suspendedUntil)}` : 'Autorisée'}</strong></div>
              </div>
              <div className={styles.field}>
              ...
              <div className={styles.adminActions}>
                ...
                {!selected.bannedAt && <button className={styles.dangerBtn} onClick={handleSuspendUser}>Bloquer temporairement</button>}
                {!selected.bannedAt && selected.suspendedUntil && selected.suspendedUntil > Date.now() && <button className={styles.secondaryBtn} onClick={handleUnsuspendUser}>Réautoriser la connexion</button>}
                {!selected.bannedAt && <button className={styles.dangerBtn} onClick={handleBanUser}>Bannir définitivement</button>}
              </div>
