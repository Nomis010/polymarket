...
export default function App() {
  ...
  const handleRequestBet = async () => {
    ...
    try {
      const data = await api('/api/bet-requests/create', { method: 'POST', token: sessionToken, body: requestForm });
      syncFromApi(data);
  ...
  const handleReport = async () => {
    ...
    try {
      const data = await api('/api/reports/create', { method: 'POST', token: sessionToken, body: reportForm });
      syncFromApi(data);
  ...
  const handleAcceptRequest = async (request) => {
    ...
    try {
      const data = await api('/api/bet-requests/accept', { method: 'POST', token: sessionToken, body: { requestId: request.id, virtualStakes } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
  ...
  const handleRejectRequest = async (requestId) => {
    try {
      const data = await api('/api/bet-requests/reject', { method: 'POST', token: sessionToken, body: { requestId } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
  ...
  const handleCloseReport = async (reportId) => {
    try {
      const data = await api('/api/reports/close', { method: 'POST', token: sessionToken, body: { reportId } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
  ...
  const handleUpdateBalance = async () => {
    ...
    try {
      const data = await api('/api/users/update', { method: 'POST', token: sessionToken, body: { username: selectedUser, balance: balanceDraft } });
      syncFromApi(data);
    } catch (error) {
      notify(error.message, 'error');
    }
